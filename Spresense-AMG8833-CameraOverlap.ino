...
void CamCB(CamImage img) {
  ...
  */
  ...
  drawpixels(dest_2d, INTERPOLATED_ROWS, INTERPOLATED_COLS
           , boxsize, boxsize, false, (uint16_t*)img.getImgBuff());
  */
  ...
  for (int y = 0; y < 240; ++y) {
    for (int x = 0; x < 240; ++x) {
      uint16_t n = x / (240/INTERPOLATED_COLS)+1;
      uint16_t m = y / (240/INTERPOLATED_ROWS);
      uint16_t camval = *(camImg + y*320 + x + 40);
      ...
      *(camImg + y*320 + x + 40) = val;
    }
  }
  for (int y = 0; y < 240; ++y) {
    for (int x = 0; x < 40; ++x) {
      *(camImg + y*320 + x) = ILI9341_BLACK;
      *(camImg + y*320 + 320-x) = ILI9341_BLACK;
    }
  }
...
void drawpixels(float *p, uint8_t rows, uint8_t cols
              , uint8_t boxWidth, uint8_t boxHeight, boolean showVal, uint16_t* camimg) {
  int colorTemp;
